...
# Refresh token if needed (skip Claude Desktop config/restart steps)
if token_expired; then
  echo "Refreshing Zoom token..."
  ./get_zoom_token.sh
  load_env
fi
...
echo "Starting Zoom MCP HTTP server on port $PORT (path: $PATH_)..."
MCP_TRANSPORT=http MCP_HTTP_PORT="$PORT" MCP_HTTP_PATH="$PATH_" node index.js
